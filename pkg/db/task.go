...
func Tasks(limit int) ([]*Task, error) {
	...
	if err != nil {
		return res, fmt.Errorf("In Tasks query row: %w\n", err)
	}
	...
	for rows.Next() {
		...
		if err != nil {
			return res, fmt.Errorf("In Tasks rows scan: %w\n", err)
		}
		res = append(res, &s)
	...
	err = rows.Err()
	if err != nil {
		return res, fmt.Errorf("In Tasks rows error: %w\n", err)
	}
...
func GetTask(id int) (*Task, error) {
	...
	if err != nil {
		return task, fmt.Errorf("In GetTask raw scan: %w\n", err)
	}

	err = raw.Err()
	if err != nil {
		return task, fmt.Errorf("In GetTask raw error : %w\n", err)
	}
...
func UpdateTask(task *Task) error {
	...
	if err != nil {
		return fmt.Errorf("Error parsing to int64 in UpdateTask: %w\n", err)
	}
	if id < 0 {
	...
	if err != nil {
		return fmt.Errorf("In UpdateTask Update error: %w\n", err)
	}
	// метод RowsAffected() возвращает количество записей к которым
	// был применена SQL команда
	count, err := res.RowsAffected()
	if err != nil {
		return fmt.Errorf("No rows udated in UpdateTask: %w\n", err)
	}
	if count == 0 {
...
func UpdateDate(nextDate string, id int) error {
	...
	if err != nil {
		return fmt.Errorf("In UpdateDate Update error: %w\n", err)
	}

	count, err := res.RowsAffected()
	if err != nil {
		return fmt.Errorf("No rows udated in UpdateDate: %w\n", err)
	}
	if count == 0 {
		return fmt.Errorf(`incorrect id for updating task`)
	}
...
func DeleteTask(id int) error {
	...
	if err != nil {
		return fmt.Errorf("Delete task error: %w\n", err)
	}
